...
if __name__ == "__main__":
    ...
    # def parquet_to_dataframe(self, folder_path):
    #     try:
    ...
    #                 if not df.empty:
    #                     dfs.append(df)
    #                     # Load DataFrame to MySQL immediately after creation
    #                     # self.load_df_to_mysql(df, self.get_database_credentials(), "student_data")
    #                 else:
    ...
    #         # Query to select only the rows that do not already exist in the database
    #         sql_query = f"SELECT * FROM {table_name} WHERE NOT EXISTS (SELECT 1 FROM {table_name} WHERE "
    #         for col in df.columns:
    #             sql_query += f"{table_name}.{col} = '{df[col].iloc[0]}' AND "  # Adjusted line
    #         sql_query = sql_query[:-4] + ")"
            
    #         logging.info(f"Executing SQL query: {sql_query}")  # Log SQL query
            
    #         # Execute the SQL query and load the result into a DataFrame
    #         existing_data = pd.read_sql(sql_query, con=engine)
    #         logging.info(f"Existing data retrieved from MySQL table '{table_name}':\n{existing_data}")
            
    #         # Concatenate the existing data with the DataFrame to upload
    #         df_to_upload = pd.concat([existing_data, df]).drop_duplicates(keep=False)
    #         logging.info("DataFrame to upload:\n{df_to_upload}")  # Log DataFrame to upload
            
    #         # Upload the filtered DataFrame to MySQL
    #         if not df_to_upload.empty:
    #             df_to_upload.to_sql(name=table_name, con=engine, if_exists=if_exists, index=False)
    #             logging.info(f"DataFrame successfully loaded into MySQL table '{table_name}'")
    #         else:
    #             logging.info("All rows from DataFrame are already present in the MySQL table. No new data uploaded.")
    #     except Exception as e:
    #         logging.error(f"An error occurred while loading DataFrame into MySQL: {e}")